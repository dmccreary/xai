...
Python is the language of AI. So we wanted to create a site that would look familiar to Python developers.  We also wanted other Python developers to be able to fork our site and customize it to their needs.
...
Note that I am running on a Mac with Apple silicon.  This means that the image libraries that brew downloads must be specific to the Mac Arm
instruction set.
...
## Generative AI Image Prompts

We are not artists here.  However, we wanted images
to convey emotions so our students could easily recall concepts.
So we try to leverage generative AI to create compelling
images that help our students with recall.
